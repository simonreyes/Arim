...
if($action == 'ajax'){
	...
	//Si selecciona a todos los transportistas, se hace una lista con TODOS
	if (strcmp($query, "ALL") == 0){
	$queryTR = "SELECT count(*) AS numrows FROM Guia as g 	WHERE Status <> 'Anulada' AND Fecha_Guia >= '".$FechaInicial."' AND Fecha_Guia <= '".$FechaFinal."' ORDER BY g.iguia";
