...
<html>
	...
	<META http-equiv=Content-Type content="text/html; charset=utf-8">
		<title>Reporte Transporte por Fechas</title>
		<style type="text/css">			
			* {
				margin:0px;
				padding:0px;
...
<div id ="block"></div>
   ...
   <p style="font-size:28px;font-family:Arial, Helvetica, sans-serif;"><font color="Blue"><b>TRANSPORTES EDGARDO CRISTIAN MORENO SOLIS E.I.R.L </b></font></p>
        <h3 class="title">Reporte Transporte Fecha Desde - Hasta</h3>
	    <div id="popupbox"></div>
        <div id="content"></div>
    </div>
	<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
   ...
   <table align="center" border="0" cellpadding="1" cellspacing="1" style="width:950px;" id="dvData">
	<tbody>
		<tr>
			...
			<td><P ALIGN=center><u><b>Distancia</b></u></P></td>
			<td><P ALIGN=center><u><b>Valor</b></u></P></td>
			<td><P ALIGN=center><u><b>Valor Total</b></u></P></td>	
			<td><P ALIGN=center><u><b>Valor Final</b></u></P></td>	
		</tr>			
		<?php 
		if($_POST){ 
		...
		conectar(); mysql_set_charset('utf8'); 
		$sqlp = "SELECT * FROM Guia WHERE Fecha_Guia  >=  '" .$busqueda. "' and Fecha_Guia  <=  '" .$busqueda2. "' order by Num_Guia";  
		...
		$CT = $filap['Folio'];
			...
			IF ($Transp == "EUGENIO ROJAS QUEZADA"){			
				$Cubo = 0;
				$Cubo = 120;
				$valor_trans = 0;
				$valor_trans = $Cubo * $KM ;				
			...
			conectar(); mysql_set_charset('utf8'); 
			$sqlddddq = "SELECT * FROM transporte WHERE nombre  =  '" .$Transp. "'";  
			$resultadoddddq = mysql_query($sqlddddq); 		
			while($filaddddq = mysql_fetch_assoc($resultadoddddq))
			{  
				$idnombre = "";
				$idnombre = $filaddddq['idnombre'];
			}
			
				
			conectar(); mysql_set_charset('utf8'); 
			$sqlddddqa = "SELECT * FROM transchofer WHERE nombretrans  =  '" .$idnombre. "' and patente  =  '" .$Patente. "'";  
			$resultadoddddqa = mysql_query($sqlddddqa); 		
			while($filaddddqa = mysql_fetch_assoc($resultadoddddqa))
			{  
				$mt3trans = "";
				$mt3trans = $filaddddqa['mt3'];
			}
		...
		<tr>
			...
			<td><P ALIGN=center><?php echo $MT3final; ?></P></td>
			...
			<td><P ALIGN=center><?php echo $Cubo; ?></P></td>
			<td><P ALIGN=center><?php echo $valor_trans; ?></P></td>
			<td><P ALIGN=center><?php echo $valor_final; ?></P></td>
		</tr>
		<?php 		
		 }  
		}
